...
var merge = function merge2(target, source, options2) {
  ...
  if (typeof source !== "object" && typeof source !== "function") {
    if (isArray$2(target)) {
      ...
      } else if (options2 && (options2.plainObjects || options2.allowPrototypes) || !has$2.call(Object.prototype, source)) {
        target[source] = true;
      }
    } else {
      return [target, source];
...
var limit = 1024;
var encode = function encode2(str, defaultEncoder, charset, kind2, format) {
  if (str.length === 0) {
    return str;
  }
  var string = str;
...
var parseObject = function(chain, val, options2, valuesParsed) {
  ...
  for (var i = chain.length - 1; i >= 0; --i) {
    ...
    if (root === "[]" && options2.parseArrays) {
      ...
      var isValidArrayIndex = !isNaN(index2) && root !== decodedRoot && String(index2) === decodedRoot && index2 >= 0 && options2.parseArrays;
      if (!options2.parseArrays && decodedRoot === "") {
        obj = { 0: leaf };
      } else if (isValidArrayIndex && index2 < options2.arrayLimit) {
        obj = [];
        obj[index2] = leaf;
...
const isOwner = (config2, { strapi }) => {
  return async (ctx, next) => {
    ...
    const protectedAPIs = await strapi.query("plugin::strapi-plugin-data-ownership-guard.plugin-setting").findMany();
    const urlPath = fullURL.pathname?.split("/")?.filter(Boolean);
    if (!urlPath?.length || urlPath.length < 2) {
      await next();
      return;
    }
    const path = urlPath?.length > 2 ? `/${urlPath[0]}/${urlPath[1]}/:id` : `/${urlPath[0]}/${urlPath[1]}`;
    const method = ctx?.request?.method;
    const params = urlPath?.length > 2 ? { id: urlPath[2] } : null;
    const uid = `api::${removeSOrES(urlPath[1])}.${removeSOrES(urlPath[1])}`;
    if (protectedAPIs.length === 0 || protectedAPIs?.filter((api) => api.path === path && api.method === method).length === 0) {
      await next();
      return;
    }
...
const collectionName = "plugin_settings";
const info = {
...
const controller = ({ strapi }) => ({
  index(ctx) {
    ctx.body = strapi.plugin("strapi-plugin-data-ownership-guard").service("service").getWelcomeMessage();
  },
  async getRoutes(ctx) {
    try {
      const routes2 = await strapi.plugin("strapi-plugin-data-ownership-guard").service("service").getRoutes();
      ctx.body = routes2;
    } catch (error) {
  ...
  async assignPolicyToRoute(ctx) {
    ...
    try {
      await strapi.plugin("strapi-plugin-data-ownership-guard").service("service").createProtectedRoute({
        path,
        method,
        handler
      });
  ...
  async getSelectedApis(ctx) {
    try {
      const apis = await strapi.plugin("strapi-plugin-data-ownership-guard").service("service").getProtectedRoutes();
      ctx.body = apis;
    } catch (error) {
  ...
  async deleteProtectedRoute(ctx) {
    const { id } = ctx.params;
    try {
      await strapi.plugin("strapi-plugin-data-ownership-guard").service("service").deleteProtectedRoute(id);
      ctx.status = 200;
    } catch (error) {
...
const service = ({ strapi }) => ({
  ...
  getRoutes() {
    const routesMap = [];
    _.forEach(strapi?.apis, async (api, apiName) => {
      const routes2 = _.flatMap(api.routes, (route) => {
  ...
  async getProtectedRoutes() {
    const routes2 = await strapi.query("plugin::strapi-plugin-data-ownership-guard.plugin-setting").findMany();
    return routes2;
  },
  async deleteProtectedRoute(id) {
    await strapi.query("plugin::strapi-plugin-data-ownership-guard.plugin-setting").delete({ where: { id } });
  },
  async createProtectedRoute(route) {
    await strapi.query("plugin::strapi-plugin-data-ownership-guard.plugin-setting").create({ data: route });
  }
});
const services = {
  service
